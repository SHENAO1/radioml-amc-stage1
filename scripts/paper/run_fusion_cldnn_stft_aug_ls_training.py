"""A 方案: fusion_cldnn_stft + signal augmentation + label smoothing 0.1.

Trains 1 model x 3 seeds on RadioML2016.10A with the SAME fixed split as
Stage 5A and the same training schedule as P1.1 / P2.5 (epoch 50, cosine LR
with 5-epoch warmup, early-stop patience 15), with three additional
interventions stacked together:

  - architecture: `fusion_cldnn_stft` (CLDNN-style I/Q encoder + STFT 2D-CNN
    branch + fused MLP classifier);
  - signal-domain augmentation, train-only (phase rotation + cyclic time shift);
  - label smoothing 0.1 in cross-entropy.

Output goes to `results/paper_stage6/fusion_cldnn_stft_aug_ls_3090/`; Stage 5A,
P1.1, P2.5, P1.2, P1.3 artefacts are not modified.
"""
...
RESULT_ROOT = Path("results/paper_stage6/fusion_cldnn_stft_aug_ls_3090/rml2016a")
STATUS_PATH = PROJECT_ROOT / RESULT_ROOT / "status.json"
REPORT_PATH = PROJECT_ROOT / "docs/paper/PAPER_STAGE6_FUSION_CLDNN_STFT_AUG_LS_3090_REPORT.md"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run A 方案 (fusion_cldnn_stft + aug + label smoothing).")
    p.add_argument("--config", default="configs/paper/rml2016a_fusion_cldnn_stft_aug_ls_3090.yaml")
...
def load_status() -> dict[str, Any]:
    ...
    return {
        "stage": "Paper-Stage 6 fusion_cldnn_stft + aug + LS (RTX 3090)",
...
def run_single(args: argparse.Namespace) -> int:
    ...
    (run_dir / "fusion_cldnn_stft_aug_ls_run_status.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def build_report(status: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Architecture: CLDNN-style I/Q encoder + STFT 2D-CNN branch + fused MLP head.",
