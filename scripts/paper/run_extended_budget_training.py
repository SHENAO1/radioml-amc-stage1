"""P1.1: extended training budget orchestrator.

Runs 4 models x 3 seeds on RadioML2016.10A with the SAME fixed split as
Stage 5A but with epochs=50 + cosine LR + 5-epoch warmup. Outputs go to a
SEPARATE root (`results/paper_stage6/extended_budget_3090/`) so Stage 5A
artifacts remain frozen.

This is a thin variant of `run_stage5a_full_training.py` with:
  - ALLOWED_MODELS reduced to the 4 P1.1 models;
  - RESULT_ROOT moved to `results/paper_stage6/extended_budget_3090/rml2016a`;
  - STATUS_PATH and REPORT_PATH moved to the new evidence root;
  - default --config pointing to the new extended-budget YAML.
"""
...
RESULT_ROOT = Path("results/paper_stage6/extended_budget_3090/rml2016a")
STATUS_PATH = PROJECT_ROOT / RESULT_ROOT / "status.json"
REPORT_PATH = PROJECT_ROOT / "docs/paper/PAPER_STAGE6_EXTENDED_BUDGET_3090_REPORT.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run P1.1 extended-budget training (4 models x 3 seeds).")
    parser.add_argument("--config", default="configs/paper/rml2016a_extended_budget_3090.yaml")
    ...
    parser.add_argument("--latency-device", default="cuda")
...
def load_status() -> dict[str, Any]:
    ...
    return {
        "stage": "Paper-Stage 6 Extended Budget (RTX 3090)",
...
def run_single(args: argparse.Namespace) -> int:
    ...
    (run_dir / "extended_budget_run_status.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def build_report(status: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Hardware: RTX 3090 24GB (Ampere sm_86), CUDA 12.8, PyTorch 2.9.1+cu128",
