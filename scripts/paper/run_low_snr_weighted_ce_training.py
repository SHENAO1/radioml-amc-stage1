"""P2.5: SNR-aware weighted CE orchestrator.

Trains 2 models (`cldnn`, `fusion_iq_stft`) x 3 seeds with the same fixed split
as Stage 5A and the same training schedule as P1.1 extended budget, but uses
`SnrWeightedCrossEntropy` (group-based weights low=2.0, mid=1.0, high=0.7).

Outputs go to `results/paper_stage6/low_snr_weighted_ce_3090/`. Stage 5A and
P1.1 artifacts are not modified.
"""
...
ALLOWED_MODELS = ["cldnn", "fusion_iq_stft"]
...
RESULT_ROOT = Path("results/paper_stage6/low_snr_weighted_ce_3090/rml2016a")
STATUS_PATH = PROJECT_ROOT / RESULT_ROOT / "status.json"
REPORT_PATH = PROJECT_ROOT / "docs/paper/PAPER_STAGE6_LOW_SNR_WEIGHTED_CE_3090_REPORT.md"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run P2.5 SNR-aware weighted CE training (2 models x 3 seeds).")
    parser.add_argument("--config", default="configs/paper/rml2016a_low_snr_weighted_ce_3090.yaml")
...
def load_status() -> dict[str, Any]:
    ...
    return {
        "stage": "Paper-Stage 6 SNR-Aware Weighted CE (RTX 3090)",
...
def run_single(args: argparse.Namespace) -> int:
    ...
    (run_dir / "low_snr_weighted_ce_run_status.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
...
def build_report(status: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"Loss: SnrWeightedCrossEntropy(scheme=group, low=2.0, mid=1.0, high=0.7).",
