...
\begin{table}[htbp]
  ...
  \end{tabular}
\end{table}
\noindent\textbf{MCLDNN 训练异常说明。}本实验中 \texttt{mcldnn} 三个训练种子表现出双峰分布：种子 42 接近正常，种子 2025 与 3407 退化至接近随机水平，导致整体准确率均值与标准差均不稳定（见第~\ref{sec:results-mcldnn-anomaly}~节）。该异常作为负面证据完整保留，不从结果表中剔除。
